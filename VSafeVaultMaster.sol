...
contract VSafeVaultMaster is IVaultMaster {
    ...
    function setSlippage(address _token, uint256 _slippage) external onlyGovernance {
        require(_slippage <= 1000, ">10%");
        slippage[_token] = _slippage;
    }

    function convertSlippage(address _input, address _output) external view override returns (uint256) {
        uint256 _is = slippage[_input];
        uint256 _os = slippage[_output];
